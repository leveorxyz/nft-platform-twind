...
const AuctionCard = ({ name, creator, bid, timeRemaining, image }: Props) => {
  return (
    <motion.div
    ...
    >
      <div className=" bg-brand-900 bg-opacity-20 backdrop-blur-xl absolute left-[50%] translate-x-[-50%] bottom-[30%] px-3 py-1.5 rounded-full">
        <div className="flex items-center gap-2">
          <svg className="h-4 w-4">
            <use xlinkHref="/images/icons/icons.svg#icon-fire" />
          </svg>
          <p className="text-xs text-center">{timeRemaining}</p>
        </div>
      </div>
